...
plot_data <- function(.data,
  ...
  sm_hod_quantiles_cat <- purrr::pmap(
    tab,
    function(x, y) {
      sm_category_list[-1] %>%
        magrittr::extract2(x, y) %>%
        unlist() %>%
        quantile(prob = quantile_prob_val, na.rm = TRUE)
    }
  ) %>%
    bind_rows() %>%
    bind_cols(tab) %>%
    rename(
      "customer_serial_id" = "x",
      "category_id" = "y"
    )

  ref_cat <- names(sm_category_list)[-c(1)] %>%
    as_tibble() %>%
    set_names("category") %>%
  ...
  sm_quantiles_ref <- sm_hod_quantiles_cat %>%
    mutate(
      customer_serial_id = as.integer(customer_serial_id),
