...
#' v2 <- suppressWarnings(dist_pairwise_clust(sm, "hour_day", quantile_prob_val = c(0.25, 0.5, 0.75)))
#' v2

dist_pairwise_clust <-  function(.data,
                              gran1 = NULL,
                              gran2 = NULL,
                              response = NULL,
  ...
  data <- unite(sm_gran, category, -c(1, 2), sep = "-") %>% rename("id_x" = "category") %>% mutate(id_facet = 1) %>% rename("sim_data"= response)
...
data_return <- df3 %>% arrange(customer_serial_id, id_x_1, id_x_2) %>%
...
data_return %>% pivot_wider(names_from = cat_comb, values_from = dist) %>% select(-1)
}
